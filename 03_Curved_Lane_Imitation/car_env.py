...
class Environment:
    def __init__(self):
        ...
        pygame.display.set_caption("Imitation Learning Driver")
    ...
    def get_human_action(self):
        action = 1 # Default action: Go Straight
        keys = pygame.key.get_pressed()
        if keys[pygame.K_LEFT]: action = 0
        elif keys[pygame.K_RIGHT]: action = 2
        
        for event in pygame.event.get():
            if event.type == pygame.QUIT: return -1 # Signal to quit
        return action
    ...
    def render(self, text_lines):
        ...
        for i in range(len(visible_points) - 1):
            p1, p2 = visible_points[i], visible_points[i+1]
            pygame.draw.polygon(self.screen, ROAD_COLOR, [(p1['x'] - ROAD_WIDTH / 2, p1['y'] - self.scroll_y), (p2['x'] - ROAD_WIDTH / 2, p2['y'] - self.scroll_y), (p2['x'] + ROAD_WIDTH / 2, p2['y'] - self.scroll_y), (p1['x'] + ROAD_WIDTH / 2, p1['y'] - self.scroll_y)])
            pygame.draw.aaline(self.screen, LINE_COLOR, (p1['x'] - ROAD_WIDTH/2, p1['y']-self.scroll_y), (p2['x'] - ROAD_WIDTH/2, p2['y']-self.scroll_y))
            pygame.draw.aaline(self.screen, LINE_COLOR, (p1['x'] + ROAD_WIDTH/2, p1['y']-self.scroll_y), (p2['x'] + ROAD_WIDTH/2, p2['y']-self.scroll_y))
            if i % 10 == 0: pygame.draw.line(self.screen, LINE_COLOR, (p1['x'], p1['y']-self.scroll_y), (p2['x'], p2['y']-self.scroll_y), 5)

        self.screen.blit(self.car_sprite, self.car.rect)
        
        for i, line in enumerate(text_lines):
            text_surface = self.font.render(line, True, LINE_COLOR)
