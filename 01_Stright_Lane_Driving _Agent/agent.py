...
# --- Configuration ---
MODEL_FILENAME = "policy_gradient_car_model_Stright_Lane.pth"
...
def test_with_gui(agent, num_episodes):
    ...
    for i_episode in range(1, num_episodes + 1):
        ...
        episode_reward = 0
        for t in range(1000):
            # In testing, we no longer need to store log_probs
            state_tensor = torch.FloatTensor(state).unsqueeze(0)
...
if __name__ == "__main__":
    ...
    train_headless(agent, num_episodes=500)
    

    test_with_gui(agent, num_episodes=5)
